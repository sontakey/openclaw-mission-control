...
- [ ] Install all npm dependencies (check package.json — if deps are missing, add: react, react-dom, react-router-dom, express, better-sqlite3, @types/better-sqlite3, @types/express, tailwindcss, lucide-react, clsx, tailwind-merge, vite, typescript, tsx, @radix-ui/react-dialog, @radix-ui/react-scroll-area, @radix-ui/react-popover, @radix-ui/react-tooltip, @radix-ui/react-collapsible, @radix-ui/react-select, @radix-ui/react-separator, @radix-ui/react-slot, class-variance-authority, framer-motion, tw-animate-css, @types/react, @types/react-dom, @vitejs/plugin-react). Run npm install. Ensure vite.config.ts has @ path alias pointing to src/.
- [ ] Implement server/gateway-client.ts — HTTP client that calls OpenClaw gateway at OPENCLAW_GATEWAY_URL (default http://127.0.0.1:18789). Uses POST /tools/invoke with Bearer OPENCLAW_TOKEN. Implement: invokeTool(tool, args), listSessions(activeMinutes?), sendToSession(sessionKey, message, timeout), getSessionHistory(sessionKey, limit), listCrons(), getConfig(). See PRD section 6 for exact implementation.
- [ ] Implement server/sse.ts — SSE broadcast manager with addClient(res), removeClient(res), broadcast(event, data) methods. Implements the event types from PRD section 8. Implement server/routes/health.ts returning { status: 'ok', uptime, timestamp }.
- [ ] Implement server/routes/tasks.ts — full CRUD: GET / (list, query params: status, assignee), GET /:id (with subtasks and comments joined), POST / (create), PATCH /:id (update fields), DELETE /:id, POST /:id/comments, POST /:id/subtasks, PATCH /:id/subtasks/:sid. Every mutation logs to activities table and calls sse.broadcast().
- [ ] Implement server/routes/agents.ts — GET /api/agents reads gateway config (getConfig -> agents.list) and merges with live sessions (listSessions). Caches for 30s. Returns array of { id, name, emoji, role, status, lastHeartbeat, currentActivity, sessionKey }. GET /api/agents/:id/sessions returns sessions for one agent.
- [ ] Implement server/routes/activities.ts — GET /api/activities (limit, offset params, ordered by created_at DESC). GET /api/activities/stream is the SSE endpoint using sse.ts addClient.
- [ ] Implement server/routes/chat.ts — POST /api/chat/send with body { sessionKey, message } proxies to gateway sendToSession. GET /api/chat/history/:sessionKey proxies to gateway getSessionHistory. Returns cleaned message array.
...
- [ ] Create src/hooks/useSSE.ts — connects to /api/activities/stream, parses SSE events, calls registered callbacks. Auto-reconnect on disconnect. Create src/hooks/useTasks.ts — fetches /api/tasks, provides createTask, updateTask, deleteTask, addComment, addSubtask, toggleSubtask. Refetches on SSE task events. Create src/hooks/useAgents.ts — fetches /api/agents, polls every 30s, provides agent list and status.
