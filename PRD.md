...
- [x] Adapt ALL src/components/ui/*.tsx files: replace every `from "@clawe/ui/lib/utils"` with `from "@/lib/utils"`. Remove any `"use client"` directives. Fix any cross-references between ui components (they should import from `./` or `@/components/ui/`). Verify no @clawe imports remain in any ui/ file.
...
- [x] Adapt src/components/live-feed/ files: replace Convex useQuery(api.activities.feed) with useActivities() hook (SSE-powered). Remove "use client". Fix all imports per ADAPTATION-GUIDE.md. Keep all UI markup and filter logic.
- [x] Adapt src/components/chat/ files: remove Clawe's WebSocket-based gateway client. Replace with simple fetch to /api/chat/send and /api/chat/history. Create a useChat(sessionKey) hook if needed. Add an agent selector dropdown at the top. Remove "use client". Keep message display UI.
...
- [x] Create src/App.tsx with React Router: BrowserRouter wrapping ThemeProvider, SquadProvider, ChatPanelProvider, DrawerProvider, and the layout shell. Routes: / -> Board page, /agents -> Agents page, /settings -> Settings page. Create src/pages/settings.tsx showing gateway connection info, cron jobs list (GET /api/gateway/crons), and dark/light theme toggle.
- [x] Update src/styles/globals.css: change brand color from rose to teal (see ADAPTATION-GUIDE.md section 8). Ensure the Clawe kanban override CSS is included. Update src/main.tsx to import globals.css and render App. Verify the full app builds with npm run build (both client and server).
- [x] Test end-to-end: start server (npm run dev:server), start client (npm run dev), verify Board loads with empty kanban, create a task via UI, see it on board, check agent status page loads, verify SSE activity stream works, verify chat panel opens. Fix any remaining TypeScript errors or runtime issues.
- [x] Create install.sh per PRD section 11, create .env.example, write README.md with architecture overview and setup instructions. Ensure .gitignore covers node_modules, dist, .env, *.db. Remove reference/ directory from the repo (it was only needed during adaptation).
